...
IntCoor = namedtuple("IntCoor", ["i", "j", "k", "l", "ij", "ijk", "ijkl"])


def sort_heavy_atoms_r(mol: Chem.Mol, idx_list: list[int]) -> None:
    """Recursively append heavy-atom neighbours to idx_list in traversal order."""
...
def add_hydrogens(mol: Chem.Mol, idx_list: list[int]) -> None:
    """Append hydrogen indices for every atom in idx_list."""
...
def get_predecessors_r(
    mol: Chem.Mol,
    atom_idx: int,
    order_map: dict[int, int],
    predecessor_list: list[int],
) -> bool:
    """Recursively collect up to 3 predecessors (lower-order atoms) for atom_idx."""
    atom = mol.GetAtomWithIdx(atom_idx)
    ...
    for neighbor in atom.GetNeighbors():
        ...
        if order_map[neighbor_idx] < order_map[atom_idx]:
            ...
            is_done = get_predecessors_r(mol, neighbor_idx, order_map, predecessor_list)
            if is_done:
                return True
...
def make_internal_map(
    mol: Chem.Mol, idx_list: list[int], order_map: dict[int, int]
) -> dict[int, list[int]]:
    """Map each atom index to its list of predecessor indices."""
    internal_map = {}
...
def calculate_internal(
    mol: Chem.Mol, idx_list: list[int], internal_map: dict[int, list[int]]
) -> list[IntCoor]:
    """Compute bond length, valence angle, and dihedral for each atom with 3 predecessors."""
    internal_coords = []
    ...
    for atom_idx in idx_list:
        ...
        if len(other_atom_idxs) == 3:
            ...
            internal_coords.append(
                IntCoor(atom_idx, *other_atom_idxs, bond, angle, torsion)
            )

    return internal_coords


def process_monomer(monomer_path: Path, coordinate_path: Path) -> None:

    mol_h = Chem.MolFromPDBFile(str(monomer_path), removeHs=False)
    ...
    idx_order_map = {
        atom_idx: order_num for order_num, atom_idx in enumerate(order_list, 1)
    }
