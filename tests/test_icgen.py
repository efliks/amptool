...
def test_process_monomer(monomer_pdb_path: Path, expected_ic_path: Path, tmp_path: Path) -> None:
...
def test_process_monomer_output_format(monomer_pdb_path: Path, tmp_path: Path) -> None:
    ...
    for line in lines:
        ...
        for i in range(4):
            assert parts[i].isalnum(), f"Expected atom label at position {i}, got {parts[i]}"
        
        # Last 3 should be numeric values (bond, angle, dihedral)
        for i in range(4, 7):
            assert parts[i].replace('.', '').replace('-', '').isdigit() or '.' in parts[i], \
